...
// GetFlowQuotaDataWithGroup is like GetFlowQuotaData but can additionally narrow
// the result to a single user group. The enterprise dimension reuses it: an
// organization is bound to a dedicated group, so filtering by group yields the
// enterprise's traffic breakdown.
func GetFlowQuotaDataWithGroup(startTime int64, endTime int64, username string, userID int, role int, group string) ([]*FlowQuotaData, error) {
	...
	case role >= common.RoleRootUser:
		return getRootFlowQuotaData(startTime, endTime, username, group)
	case role >= common.RoleAdminUser:
		return getAdminFlowQuotaData(startTime, endTime, username, group)
	default:
...
func getAdminFlowQuotaData(startTime int64, endTime int64, username string, group string) ([]*FlowQuotaData, error) {
	rows := make([]*FlowQuotaData, 0)
...
func getRootFlowQuotaData(startTime int64, endTime int64, username string, group string) ([]*FlowQuotaData, error) {
	rows := make([]*FlowQuotaData, 0)
	query := flowQuotaBaseQuery(startTime, endTime).
